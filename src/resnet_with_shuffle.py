...
def reshuffle_dataset(train_data_dir, val_data_dir, train_ratio=0.6):
    # Create new train and validation directories
    new_train_dir = os.path.join(script_dir, '../dataset1/train')
    new_val_dir = os.path.join(script_dir, '../dataset1/validation')
    
    for severity in range(5):
        os.makedirs(os.path.join(new_train_dir, str(severity)), exist_ok=True)
        os.makedirs(os.path.join(new_val_dir, str(severity)), exist_ok=True)

        # Get the image paths for the current severity level from both train and validation directories
        train_severity_dir = os.path.join(train_data_dir, str(severity))
        val_severity_dir = os.path.join(val_data_dir, str(severity))
        
        train_images = [os.path.join(train_severity_dir, fname) for fname in os.listdir(train_severity_dir)]
        val_images = [os.path.join(val_severity_dir, fname) for fname in os.listdir(val_severity_dir)]
        
        # Combine both train and validation images for the current severity level
        all_images = train_images + val_images
        
        # Shuffle the combined images for this severity level
        random.shuffle(all_images)
        
        # Calculate the new split index
        split_index = int(len(all_images) * train_ratio)
        
        # Copy images to the new train and validation directories
        for i, img_path in enumerate(all_images):
            severity_folder = str(severity)
            destination_dir = new_train_dir if i < split_index else new_val_dir
            destination_path = os.path.join(destination_dir, severity_folder)
            
            # Check if the image already exists in the destination directory
            if not os.path.exists(os.path.join(destination_path, os.path.basename(img_path))):
                shutil.copy(img_path, destination_path)  # Copy the image to the correct folder
            else:
                print(f"File {img_path} already exists at destination. Skipping.")
...
reshuffle_dataset(train_data_dir, val_data_dir)
